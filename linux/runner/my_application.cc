...
static void my_application_activate(GApplication* application) {
  ...
  // Register the Lucent native input-lock plugin. It needs the binary
  // messenger from the engine and the top-level window to grab/inhibit input
  // on. The window is already realized above (gtk_widget_realize), so its
  // GdkWindow exists by the time lock() is called.
